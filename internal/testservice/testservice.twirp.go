...
func (c *testServiceProtobufClient) Fail(ctx context.Context, in *FailRequest) (*FailResponse, error) {
	...
	caller := c.callFail
	if c.interceptor != nil {
		caller = func(ctx context.Context, req *FailRequest) (*FailResponse, error) {
			resp, err := c.interceptor(
				func(ctx context.Context, req interface{}) (interface{}, error) {
					typedReq, ok := req.(*FailRequest)
					if !ok {
						return nil, twirp.InternalError("failed type assertion req.(*FailRequest) when calling interceptor")
					}
					return c.callFail(ctx, typedReq)
				},
			)(ctx, req)
			if resp != nil {
				typedResp, ok := resp.(*FailResponse)
				if !ok {
					return nil, twirp.InternalError("failed type assertion resp.(*FailResponse) when calling interceptor")
				}
				return typedResp, err
			}
			return nil, err
...
func (s *testServiceServer) serveFail(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveFailJSON(ctx, resp, req)
	case "application/protobuf":
		s.serveFailProtobuf(ctx, resp, req)
	default:
		msg := fmt.Sprintf("unexpected Content-Type: %q", req.Header.Get("Content-Type"))
		twerr := badRouteError(msg, req.Method, req.URL.Path)
		s.writeError(ctx, resp, twerr)
